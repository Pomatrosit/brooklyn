<template>
  <div>
    <form @submit.prevent="onSubmit" v-if="formStep === 1">
      ...
      <p class="subtitle">
        <strong>Отправьте заявку</strong> и наш менеджер<br />
        свяжется с вами
      </p>
      ...
      <div class="submit-btn-wrapper">
        ...
        />
        <div v-if="isErrorMessage" class="error-message">
          Поля заполненны некорректно, попробуйте ввести данные повторно
        </div>
      </div>
      <div class="policy">
        <div class="policy__checkbox" @click="toggleCheckbox">
          ...
          >
            <path
              d="M15 1.59999L13.5 0L3.74996 10.3999L5.24997 11.9999L15 1.59999Z"
              fill="#242135"
            />
            <path
              d="M1.50025 4.80006L0.000244141 6.40005L5.24997 11.9999L6.75027 10.4L1.50025 4.80006Z"
              fill="#242135"
            />
          </svg>
        </div>
  ...
  methods: {
    ...
    onSubmit() {
      ...
      if (
        ...
        axios.get(
          `${API_URL}/api/mail/${payload.name}/${this.phone}/${payload.type}`
        );
        axios
          .post(`${API_URL}/api/order`, payload)
        ...
        if (this.name.length < 2)
          this.$refs.nameInput.style.border = "1px solid #BB2929";
        if (rawValue === "" || rawValue.split("").includes("_"))
          this.$refs.phoneInput.$el.style.border = "1px solid #BB2929";
      }
    },
    toggleCheckbox() {
